...
import by.pressf.emailnotificationms.kafka.listener.REmailWUserEventsListener;
...
public class REmailWUserEventsListenerUnitTests {
    private @Mock EmailService emailService;
    private @Mock IdempotencyService idempotencyService;
    private @InjectMocks REmailWUserEventsListener listener;

    @ParameterizedTest @MethodSource("arguments")
    void handle_Success_ReturnsNormally(BalanceTopUpFailedEvent event,
                                        String messageId) {
        // Arrange
    ...
    @ParameterizedTest @MethodSource("arguments")
    void handle_DuplicateMessage_SwallowsException(BalanceTopUpFailedEvent event,
                                                   String messageId) {
        // Arrange
    ...
    @ParameterizedTest @MethodSource("arguments")
    void handle_MailSendError_ThrowsRetryableException(BalanceTopUpFailedEvent event,
                                                       String messageId) {
        // Arrange
        doNothing().when(idempotencyService).idempotenceCheck(anyString(), anyString());
    ...
    @ParameterizedTest @MethodSource("arguments")
    void handle_GenericMailError_ThrowsNotRetryableException(BalanceTopUpFailedEvent event,
                                                             String messageId) {
        // Arrange
        doNothing().when(idempotencyService).idempotenceCheck(anyString(), anyString());
    ...
    @ParameterizedTest @MethodSource("arguments")
    void handle_DataAccessError_ThrowsNotRetryableException(BalanceTopUpFailedEvent event,
                                                            String messageId) {
        // Arrange
        doNothing().when(idempotencyService).idempotenceCheck(anyString(), anyString());
        doNothing().when(emailService).sendEmail(anyString(), anyString(), anyString());
    ...
    private static Stream<Arguments> arguments() {
        return Stream.of(
                Arguments.of(new BalanceTopUpFailedEvent("test@mail.com", "title",
                        "text"), "msg-123-test")
        );
    }
}
